...
class MapPaymentTransaction
{
    ...
    public function handle($event)
    {
        ...
        if (isset($event->isDeleted) && $event->isDeleted) {
            ...
            try {
                if (! $accountingUtil->deleteMap($payment->business_id, $payment->transaction_id, $payment->id)) {
                    \Log::warning('Accounting: deleteMap skipped (period locked)', ['type' => 'payment', 'payment_id' => $payment->id]);
                }
            } catch (\Throwable $e) {
        ...
        if(!isset($event->isDeleted) || !$event->isDeleted){
            ...
            if (! is_null($deposit_to) && ! is_null($payment_account)) {
                ...
                $accountingUtil = new \Modules\Accounting\Utils\AccountingUtil();
                try {
                    if (! $accountingUtil->saveMap($type, $payment_id, $user_id, $business_id, $deposit_to, $payment_account)) {
                        \Log::warning('Accounting: saveMap skipped (period locked)', ['type' => $type, 'payment_id' => $payment_id]);
                    }
                } catch (\Throwable $e) {
                    \Log::error('Accounting saveMap failed', ['type' => $type, 'message' => $e->getMessage()]);
                }
            }
        }
    }
