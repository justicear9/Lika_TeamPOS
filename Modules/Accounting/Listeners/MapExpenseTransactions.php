...
class MapExpenseTransactions
{
    ...
    public function handle($event)
    {
        ...
        if (isset($event->isDeleted) && $event->isDeleted) {
            try {
                if (! $accountingUtil->deleteMap($business_id, $event->expense->id, null)) {
                    \Log::warning('Accounting: deleteMap skipped (period locked)', ['type' => 'expense', 'expense_id' => $event->expense->id]);
                }
            } catch (\Throwable $e) {
            ...
            if (! is_null($deposit_to) && ! is_null($payment_account)) {
                ...
                try {
                    if (! $accountingUtil->saveMap($type, $id, $user_id, $business_id, $deposit_to, $payment_account)) {
                        \Log::warning('Accounting: saveMap skipped (period locked)', ['type' => 'expense', 'expense_id' => $id]);
                    }
                } catch (\Throwable $e) {
