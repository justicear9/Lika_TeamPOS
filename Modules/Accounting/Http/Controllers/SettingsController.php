...
class SettingsController extends Controller
{
    ...
    public function resetData()
    {
        ...
        $bankIds = AccountingBankAccount::where('business_id', $business_id)->pluck('id');
        if ($bankIds->isNotEmpty()) {
            AccountingBankStatementLine::whereIn('bank_account_id', $bankIds)->delete();
        }
        AccountingBankAccount::where('business_id', $business_id)->delete();
        AccountingAuditLog::where('business_id', $business_id)->delete();

        AccountingFixedAsset::where('business_id', $business_id)->delete();
    ...
    public function saveSettings(Request $request)
    {
        ...
        try {
            ...
            if (array_key_exists('fixed_asset_code_prefix', $incoming)) {
                $incoming['fixed_asset_code_prefix'] = $this->accountingUtil->sanitizedFixedAssetCodePrefix(
                    $incoming['fixed_asset_code_prefix'] ?? ''
                );
            }
            $merged = array_merge($existing, $incoming);

            Business::where('id', $business_id)
                        ->update(['accounting_settings' => json_encode($merged)]);
