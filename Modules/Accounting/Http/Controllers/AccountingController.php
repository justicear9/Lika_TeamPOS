...
class AccountingController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $start_date = request()->get('start_date', $financial_year['start'] ?? date('Y-m-d'));
        $end_date = request()->get('end_date', $financial_year['end'] ?? date('Y-m-d'));
        ...
        $coa_overview = AccountingAccount::leftjoin('accounting_accounts_transactions as AAT',
                                    function($join) use ($start_date, $end_date) {
                                        $join->on('AAT.accounting_account_id', '=', 'accounting_accounts.id')
                                             ->whereNotNull('AAT.operation_date')
