...
class CoaController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $input['is_cash_account'] = $request->boolean('is_cash_account');

            $account = AccountingAccount::find($id);
            $before = $account->only(['name', 'gl_code', 'account_primary_type', 'is_cash_account']);
            $account->update($input);

            \Modules\Accounting\Services\AccountingAuditService::log(
                $business_id,
                auth()->id(),
                'account.updated',
                ...
                (int) $account->id,
                $before,
                $account->fresh()->only(['name', 'gl_code', 'account_primary_type', 'is_cash_account'])
            );
    ...
    public function ledger($account_id)
    {
        ...
        $current_bal_q = AccountingAccount::leftjoin('accounting_accounts_transactions as AAT',
                            'AAT.accounting_account_id', '=', 'accounting_accounts.id')
                        ->where('business_id', $business_id)
                        ->where('accounting_accounts.id', $account->id);
        $location_id = request()->input('location_id');
        if ($location_id !== null && $location_id !== '') {
            $current_bal_q->where('AAT.location_id', $location_id);
        }
        $current_bal = $current_bal_q->select([DB::raw($this->accountingUtil->balanceFormula())])
                        ->first()->balance;

        $business_locations = \App\BusinessLocation::forDropdown($business_id, true);

        return view('accounting::chart_of_accounts.ledger')
            ->with(compact('account', 'current_bal', 'business_locations', 'location_id'));
    }
}
