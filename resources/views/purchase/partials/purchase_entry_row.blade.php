@foreach( $variations as $variation)
    <tr @if(!empty($purchase_order_line)) data-purchase_order_id="{{$purchase_order_line->transaction_id}}" @endif @if(!empty($purchase_requisition_line)) data-purchase_requisition_id="{{$purchase_requisition_line->transaction_id}}" @endif>
        ...
        <td>
            ...
            @php
                $imported_sell_price = ! empty($imported_data['default_sell_price']) ? $imported_data['default_sell_price'] : null;
                $row_sell_price_inc_tax = $imported_sell_price ?? $variation->sell_price_inc_tax;
            @endphp
            <input type="hidden" class="base_unit_cost" value="{{$variation->default_purchase_price}}">
            <input type="hidden" class="base_unit_selling_price" value="{{$row_sell_price_inc_tax}}">
        ...
        <td class="@if(!session('business.enable_editing_product_from_purchase') || !empty($is_purchase_order)) hide @endif">
            @php
                ...
                if (! empty($imported_sell_price) && ! empty($pp_without_discount) && $pp_without_discount > 0) {
                    $profit_percent = (($row_sell_price_inc_tax - $pp_without_discount) * 100) / $pp_without_discount;
                }
            @endphp
        ...
        <td>
            @if(session('business.enable_editing_product_from_purchase'))
                {!! Form::text('purchases[' . $row_count . '][default_sell_price]', number_format($row_sell_price_inc_tax, $currency_precision, $currency_details->decimal_separator, $currency_details->thousand_separator), ['class' => 'form-control input-sm input_number default_sell_price', 'required']); !!}
            @else
                {{ number_format($row_sell_price_inc_tax, $currency_precision, $currency_details->decimal_separator, $currency_details->thousand_separator)}}
            @endif
        </td>
