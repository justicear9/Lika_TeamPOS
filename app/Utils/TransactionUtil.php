...
class TransactionUtil extends Util
{
    ...
    public function calculatePaymentStatus($transaction_id, $final_amount = null)
    {
        ...
        // Avoid false "partial" status from fraction-of-cent differences
        // introduced by imported totals vs payment input precision.
    ...
    public function getListSells($business_id, $sale_type = 'sell')
    {
        $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
                ...
                ->select(
                    ...
                    DB::raw('(SELECT COUNT(*) FROM transaction_sell_lines tsl_disc WHERE tsl_disc.transaction_id = transactions.id AND tsl_disc.parent_sell_line_id IS NULL AND COALESCE(tsl_disc.line_discount_amount, 0) > 0) as sell_lines_with_discount'),
                    DB::raw('(CASE WHEN COALESCE(transactions.discount_amount, 0) > 0 OR EXISTS (
                        SELECT 1 FROM transaction_sell_lines tsl_hd
                        WHERE tsl_hd.transaction_id = transactions.id
                        AND tsl_hd.parent_sell_line_id IS NULL
                        AND COALESCE(tsl_hd.line_discount_amount, 0) > 0
                    ) THEN 1 ELSE 0 END) as has_discount')
                );

        if ($sale_type == 'sell') {
            $pendingSub = 'approval_pending';
            $sells->where(function ($q) use ($pendingSub) {
                $q->where('transactions.status', 'final')
                    ->orWhere(function ($q2) use ($pendingSub) {
                        $q2->where('transactions.status', 'draft')
                            ->where('transactions.sub_status', $pendingSub);
                    });
            });
        }
    ...
    public function addSellReturn($input, $business_id, $user_id, $uf_number = true)
    {
        ...
        if (empty($sell_return)) {
            ...
            $sell_return_data['status'] = $input['status'] ?? 'final';
            $sell_return_data['sub_status'] = $input['sub_status'] ?? null;
            ...
            if (! empty($input['status'])) {
                $sell_return_data['status'] = $input['status'];
            }
            if (array_key_exists('sub_status', $input)) {
                $sell_return_data['sub_status'] = $input['sub_status'];
            }
        ...
        foreach ($product_lines as $index => $product_line) {
            $sell_line_id = ! empty($product_line['sell_line_id']) ? (int) $product_line['sell_line_id'] : null;
            if (empty($sell_line_id) && isset($parent_sell_lines[$index])) {
                // Some modules normalize line payloads and drop sell_line_id. Fall back to row order from parent sale.
                $sell_line_id = (int) $parent_sell_lines[$index]->id;
            }
            if (empty($sell_line_id)) {
                continue;
            }
            $returns[$sell_line_id] = $uf_number ? $this->num_uf($product_line['quantity']) : $product_line['quantity'];
        }
    ...
    /**
     * Finalize a sell return that was saved as non-final for approval workflow.
     */
    public function completeSellReturnAfterApproval(int $sell_return_id, array $input, int $business_id, int $user_id, bool $uf_number = true)
    {
        $sell_return = Transaction::findOrFail($sell_return_id);
        $business = Business::with(['currency'])->findOrFail($business_id);
        $productUtil = new \App\Utils\ProductUtil();
