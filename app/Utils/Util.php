...
class Util
{
    ...
    /**
     * Blade @format_date — must return an expression usable inside {{ }} (no raw <?php blocks).
     */
    public static function bladeFormatDate($value): string
    {
        if ($value === null || $value === '') {
            return '';
        }
        try {
            ...
            $fmt = session('business.date_format', 'Y-m-d');
    ...
    /**
     * Blade @format_time — expression form for {{ }}.
     */
    public static function bladeFormatTime($value): string
    {
        if ($value === null || $value === '') {
            return '';
        }
        try {
            $d = \Carbon\Carbon::parse($value);
            $tf = session('business.time_format') == 24 ? 'H:i' : 'h:i A';
    ...
    /**
     * Blade @format_datetime — expression form for {{ }}.
     */
    public static function bladeFormatDatetime($value): string
    {
        if ($value === null || $value === '') {
            return '';
        }
