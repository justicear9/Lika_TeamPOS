...
class ModuleUtil extends Util
{
    ...
    /**
     * Merge transaction payload overrides from installed modules (e.g. approval workflow).
     * DataController implementations return ['input' => key-value overrides].
     *
     * @param  \Illuminate\Http\Request|null  $request
     */
    public function mergeTransactionInputFromModules(array $input, $request, string $context, ?int $businessId = null): array
    {
        $businessId = $businessId ?? (int) (session()->get('user.business_id') ?? 0);
        if ($businessId < 1) {
            return $input;
        }
        ...
        foreach ($modules as $module => $details) {
            ...
            $class = 'Modules\\'.$details['name'].'\Http\Controllers\DataController';
            if (! class_exists($class)) {
                continue;
            }

            $classObject = new $class();
            if (! method_exists($classObject, 'mutate_transaction_input_for_approval')) {
                continue;
            }

            try {
                $out = call_user_func([$classObject, 'mutate_transaction_input_for_approval'], $args);
                if (is_array($out) && ! empty($out['input']) && is_array($out['input'])) {
                    $input = array_merge($input, $out['input']);
                    $args['input'] = $input;
                }
            } catch (\Throwable $e) {
                \Log::warning('mutate_transaction_input_for_approval: '.$e->getMessage());
            }
        }
