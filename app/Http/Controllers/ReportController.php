...
class ReportController extends Controller
{
    ...
    public function getproductSellReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            $payment_types = $this->transactionUtil->payment_types(null, true, $business_id);
            $include_returns = $request->boolean('include_returns', true);
            ...
            return Datatables::of($query)
                ...
                ->addColumn('returned_qty', function ($row) {
                    return '<span class="returned_qty" data-orig-value="'.$row->returned_qty.'" data-unit="'.$row->unit.'" >'.
                    $this->transactionUtil->num_f($row->returned_qty, false, null, true).'</span> '.$row->unit;
                })
                 ->editColumn('subtotal', function ($row) {
                     //ignore child sell line of combo product
                ...
                ->rawColumns(['invoice_no', 'unit_sale_price', 'subtotal', 'sell_qty', 'returned_qty', 'discount_amount', 'unit_price', 'tax', 'customer', 'payment_methods'])
                ->make(true);
        }
        ...
        if ($request->ajax()) {
            ...
            $query = TransactionSellLine::join(
                ...
                ->select(
                    ...
                    'pl.lot_number',
                    'tspl.qty_returned as returned_qty'
                );
            ...
            return Datatables::of($query)
                ...
                ->addColumn('returned_qty', function ($row) {
                    return '<span data-is_quantity="true" class="display_currency returned_qty" data-currency_symbol=false data-orig-value="'.(float) $row->returned_qty.'" data-unit="'.$row->unit.'" >'.(float) $row->returned_qty.'</span> '.$row->unit;
                })
                ->editColumn('ref_no', '
                    ...
                    ')
                ->editColumn('customer', '@if(!empty($supplier_business_name)) {{$supplier_business_name}},<br>@endif {{$customer}}')
                ->rawColumns(['invoice_no', 'purchase_quantity', 'returned_qty', 'ref_no', 'customer'])
                ->make(true);
        }
    }
        ...
        if ($request->ajax()) {
            ...
            $query = TransactionSellLine::join(
                ...
                ->select(
                    ...
                    DB::raw('SUM(transaction_sell_lines.quantity_returned) as returned_qty'),
                    'u.short_name as unit',
            ...
            return Datatables::of($query)
                ...
                ->rawColumns(['current_stock', 'subtotal', 'total_qty_sold', 'returned_qty'])
                ->make(true);
        }
    }
    ...
    public function productSellReportBy(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($query)
                ...
                ->addColumn('returned_qty', function ($row) {
                    return '<span data-is_quantity="true" class="display_currency returned_qty" data-currency_symbol=false data-orig-value="'.(float) $row->returned_qty.'" data-unit="">'.(float) $row->returned_qty.'</span>';
                })
                ->editColumn('current_stock', function ($row) {
                ...
                ->rawColumns(['current_stock', 'subtotal', 'total_qty_sold', 'returned_qty', 'category_name'])
                ->make(true);
        }
    }
