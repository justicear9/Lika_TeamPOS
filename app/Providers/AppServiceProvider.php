...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        // Expression-only (no <?php) so `{{ @format_date('now') }}` compiles to valid PHP.
        Blade::directive('format_date', function ($date) {
            return "\\App\\Utils\\Util::bladeFormatDate({$date})";
        });

        Blade::directive('format_time', function ($date) {
            return "\\App\\Utils\\Util::bladeFormatTime({$date})";
        });

        Blade::directive('format_datetime', function ($date) {
            return "\\App\\Utils\\Util::bladeFormatDatetime({$date})";
        });
